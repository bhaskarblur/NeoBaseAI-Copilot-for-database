...
// InjectCursorIntoMongoQuery handles cursor-based pagination for MongoDB find() queries.
//
// Two modes:
//
//   - cursor == "" (initial page): strips the {{cursor_value}} filter from .find() so the
//     query returns the first page naturally (no assumptions about field names or types).
//
//   - cursor != "" (subsequent pages): replaces the {{cursor_value}} placeholder with the
//     correct BSON value. 24-hex strings are wrapped as ObjectId("…"); ISO-8601 timestamps
//     are wrapped as ISODate("…"); everything else is left as a quoted string.
//     processMongoDBQueryParams (called by the driver) then converts those to native BSON types.
//
// The function is DB-layer code and lives here alongside processMongoDBQueryParams / processObjectIds
// rather than in the service layer.
func InjectCursorIntoMongoQuery(query, cursor string) string {
	...
	if cursor == "" {
		// Initial page: strip the .find(FILTER, ...) first argument that contains the
		// placeholder so MongoDB scans from the beginning of the collection.
		return mongoStripCursorFilter(query)
	}

	// Format the cursor value as the correct BSON literal.
	// processMongoDBQueryParams in this file will later convert ObjectId("…") / ISODate("…")
	// into {"$oid":"…"} / {"$date":"…"}, which processObjectIds turns into native BSON types.
	replacement := mongoFormatCursorValue(cursor)

	// Replace quoted forms first so we don't double-quote: 'placeholder' and "placeholder"
	query = strings.Replace(query, "'"+placeholder+"'", replacement, -1)
	query = strings.Replace(query, `"`+placeholder+`"`, replacement, -1)
	// Then bare (shouldn't happen in well-formed LLM output, but be safe)
	query = strings.Replace(query, placeholder, replacement, -1)
	return query
}

// mongoFormatCursorValue returns the MongoDB BSON literal representation of a cursor value.
// 24-hex strings → ObjectId("…"), ISO-8601 datetimes → ISODate("…"), else quoted string.
func mongoFormatCursorValue(v string) string {
	// ISO 8601 datetime: "2025-10-30T10:44:34.978Z"
	if len(v) >= 20 && v[4] == '-' && v[7] == '-' && v[10] == 'T' {
		return fmt.Sprintf(`ISODate("%s")`, v)
	}
...
// mongoStripCursorFilter replaces the {{cursor_value}}-containing filter argument in a MongoDB
// .find(FILTER, projection) call with {} so the first page is fetched without any cursor condition.
func mongoStripCursorFilter(query string) string {
	const placeholder = "{{cursor_value}}"
	...
	// Find the opening brace of the filter argument.
	filterOpen := -1
	for i := findIdx + 6; i < len(query); i++ {
		ch := query[i]
		if ch == '{' {
			filterOpen = i
			break
	...
	depth, filterClose := 0, -1
	for i := filterOpen; i < len(query); i++ {
		switch query[i] {
		case '{':
			depth++
		case '}':
	...
	filterStr := query[filterOpen : filterClose+1]
	if !strings.Contains(filterStr, placeholder) {
		return query
	}

	// Surgically remove ONLY the cursor key-value pair, preserving all other filters.
	stripped := mongoRemoveCursorCondition(filterStr)
	return query[:filterOpen] + stripped + query[filterClose+1:]
}

// mongoRemoveCursorCondition removes only the key: {$op: '{{cursor_value}}'} pair from
// a MongoDB filter object string, leaving all other conditions intact.
// Examples:
//
//	"{createdAt: {$gt: '{{cursor_value}}'}}"                         → "{}"
//	"{status: 'ACTIVE', createdAt: {$gt: '{{cursor_value}}'}}"      → "{status: 'ACTIVE'}"
//	"{createdAt: {$gt: '{{cursor_value}}'}, active: true}"           → "{active: true}"
//	"{a: 1, createdAt: {$gt: '{{cursor_value}}'}, b: 2}"            → "{a: 1, b: 2}"
func mongoRemoveCursorCondition(filter string) string {
	const placeholder = "{{cursor_value}}"

	placeholderPos := strings.Index(filter, placeholder)
	if placeholderPos == -1 {
		return filter
	}

	// Walk back from placeholder to find the '{' that opens the value object {$op: ...}
	valueStart := -1
	for i := placeholderPos - 1; i >= 0; i-- {
		if filter[i] == '{' {
			valueStart = i
			break
		}
	...
	if valueStart == -1 {
		return "{}" // unexpected format — safe fallback
	}

	// Walk forward to find the matching '}' of the value object.
	depth, valueEnd := 0, -1
	for i := valueStart; i < len(filter); i++ {
		switch filter[i] {
		case '{':
			depth++
		case '}':
	...
	// Walk back from valueStart to find the key name.
	// Skip ':' and surrounding whitespace first.
	i := valueStart - 1
	for i >= 0 && (filter[i] == ' ' || filter[i] == '\t' || filter[i] == '\n' || filter[i] == ':') {
		i--
	}
	if i < 0 {
		return "{}"
	...
	// Key may be quoted ('field' or "field") or bare (field).
	var keyStart int
	if filter[i] == '\'' || filter[i] == '"' {
		quoteChar := filter[i]
		i--
		for i >= 0 && filter[i] != quoteChar {
			i--
		}
		keyStart = i // opening quote
	} else {
		// Bare identifier: walk back while alphanumeric / _ / $
		for i >= 0 && mongoIsIdentChar(filter[i]) {
			i--
		}
		keyStart = i + 1
	}

	// Determine removal range, handling surrounding commas in all four cases.
	// Walk back past whitespace to check for a preceding comma.
	beforeKey := keyStart - 1
	for beforeKey >= 0 && (filter[beforeKey] == ' ' || filter[beforeKey] == '\t' || filter[beforeKey] == '\n') {
		beforeKey--
	}
	// Walk forward past whitespace after the value to check for a trailing comma.
	afterValue := valueEnd + 1
	for afterValue < len(filter) && (filter[afterValue] == ' ' || filter[afterValue] == '\t' || filter[afterValue] == '\n') {
		afterValue++
	}

	var removeFrom, removeTo int
	if beforeKey >= 0 && filter[beforeKey] == ',' {
		// There is a field BEFORE this one — eat the leading comma too.
		removeFrom = beforeKey
		removeTo = valueEnd + 1
	} else if afterValue < len(filter) && filter[afterValue] == ',' {
		// This is the first field and there are more after — eat the trailing comma.
		removeFrom = keyStart
		removeTo = afterValue + 1
	} else {
		// Only condition in the filter.
		removeFrom = keyStart
		removeTo = valueEnd + 1
	}

	result := filter[:removeFrom] + filter[removeTo:]

	// If the braces now contain only whitespace, normalise to {}.
...
func mongoIsIdentChar(c byte) bool {
