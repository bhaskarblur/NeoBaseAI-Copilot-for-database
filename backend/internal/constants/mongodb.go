  ...
  "queries": [
    {
      ...
      "pagination": {
          "paginatedQuery": "CURSOR-BASED (preferred for find queries on large datasets): use '{{cursor_value}}' placeholder in the cursor field filter. cursor_field MUST be a field included in the result (not excluded via projection :0). Example: db.users.find({createdAt: {$gt: '{{cursor_value}}'}}, {name:1,email:1,createdAt:1}).sort({createdAt: 1}).limit(50). OFFSET-BASED (fallback only for aggregate pipelines without a natural cursor): .find(filter, projection).skip(offset_size).limit(50). Set cursor_field to empty string for offset mode. Set to EMPTY STRING when user requests fewer than 50 records or query already has a small limit().",
