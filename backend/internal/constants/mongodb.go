  ...
  "queries": [
    {
      ...
      "pagination": {
          "paginatedQuery": "This is the query for SUBSEQUENT PAGES (page 2, 3, etc) — NOT for the first page. The 'query' field above is used for the first page and MUST NOT contain {{cursor_value}} or offset_size. CURSOR-BASED (preferred for find queries): use '{{cursor_value}}' placeholder in the cursor field filter. cursor_field MUST be a field included in the result (not excluded via projection :0). Example: db.users.find({createdAt: {$gt: '{{cursor_value}}'}}, {name:1,email:1,createdAt:1}).sort({createdAt: 1}).limit(50). OFFSET-BASED (for aggregate pipelines without a natural cursor): use 'offset_size' as the skip value. For .find(): db.users.find({}).skip(offset_size).limit(50). For .aggregate(): db.users.aggregate([..., {$skip: offset_size}, {$limit: 50}]). IMPORTANT: Use 'offset_size' (NOT '{{cursor_value}}') for $skip-based pagination. Set cursor_field to empty string for offset mode. Set to EMPTY STRING when user requests fewer than 50 records or query already has a small limit(). IMPORTANT: The 'query' field must be the SAME query but WITHOUT the cursor/offset/skip condition.",
          "cursor_field": "Collection field used as the pagination cursor (e.g. 'createdAt', '_id'). MUST be present in the projected result (not excluded by projection). Leave EMPTY STRING when using offset-based pagination.",
          "page_size": 50,
