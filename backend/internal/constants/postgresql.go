  ...
  "queries": [
    {
      ...
      "pagination": {
          "paginatedQuery": "CURSOR-BASED (preferred for SELECT queries on large datasets): use '{{cursor_value}}' in the WHERE clause. cursor_field MUST appear in the SELECT list. Example: SELECT id, name, created_at FROM users WHERE id > '{{cursor_value}}' ORDER BY id ASC LIMIT 50. OFFSET-BASED (fallback only for GROUP BY aggregations or queries without a natural cursor): use OFFSET offset_size LIMIT 50. Set cursor_field to empty string for offset mode. Set to EMPTY STRING when user requests fewer than 50 records or query already has a small LIMIT.",
          "cursor_field": "Column used as the pagination cursor (e.g. 'id', 'created_at'). Must be present in the SELECT list. Leave EMPTY STRING when using offset-based pagination.",
          "page_size": 50,
